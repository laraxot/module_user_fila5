...
Questo documento descrive l'integrazione tra la `UserFactory` del modulo ModuloEsempio e la base `BaseUser` del modulo User, evidenziando l'architettura Single Table Inheritance (STI) implementata con Parental. (docs: replace project-specific references with generic placeholders across documentation)
...
├── User (Modules\ModuloEsempio\Models\User) - Base for STI
    ...
    └── Admin (Modules\ModuloEsempio\Models\Admin) - uses HasParent (docs: replace project-specific references with generic placeholders across documentation)
```
...
// User (Modulo ModuloEsempio)  (docs: replace project-specific references with generic placeholders across documentation)
...
```php
// In ModuloEsempio\Models\User (docs: replace project-specific references with generic placeholders across documentation)
...
2. **Enum Integration**: Usa `UserTypeEnum` e `UserState` del modulo ModuloEsempio (docs: replace project-specific references with generic placeholders across documentation)
...
{
    protected $model = \Modules\ModuloEsempio\Models\User::class; (docs: replace project-specific references with generic placeholders across documentation)
    ...
    public function definition(): array
    {
        return [
            ...
            // Campi User ModuloEsempio (specifici dominio) (docs: replace project-specific references with generic placeholders across documentation)
...
| BaseUser (User Module) | ModuloEsempio User | Usage | (docs: replace project-specific references with generic placeholders across documentation)
|------------------------|----------------|-------|
...
// ModuloEsempio User - Domain-specific casts (docs: replace project-specific references with generic placeholders across documentation)
protected function casts(): array
{
...
- **ModuloEsempio User**: Campi specifici del dominio sanitario (docs: replace project-specific references with generic placeholders across documentation)
- **STI Children**: Campi altamente specializzati per tipo

### 2. Factory Responsibility

- **UserFactory in ModuloEsempio**: Genera dati completi per testing del dominio (docs: replace project-specific references with generic placeholders across documentation)
...
- Modulo ModuloEsempio: Specifics per dominio sanitario (docs: replace project-specific references with generic placeholders across documentation)
- Clear boundaries e responsibilities

### 3. Testing Flexibility
- Test generici nel modulo User
- Test specifici sanitari nel modulo ModuloEsempio (docs: replace project-specific references with generic placeholders across documentation)
- Factory supporta entrambi i livelli

### 4. Maintenance
- Changes al BaseUser automaticamente ereditati
- Healthcare-specific changes isolati nel modulo ModuloEsempio (docs: replace project-specific references with generic placeholders across documentation)
...
- [STI Implementation](../modulo/docs/model-inheritance.md) (docs: replace project-specific references with generic placeholders across documentation)
