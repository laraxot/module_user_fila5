# UserFactory Integration - Modulo User e Application

## Overview

Questo documento descrive l'integrazione tra la `UserFactory` del modulo Application e la base `BaseUser` del modulo User, evidenziando l'architettura Single Table Inheritance (STI) implementata con Parental.
...
├── User (Modules\Application\Models\User) - Base for STI
    ├── Patient (Modules\Application\Models\Patient) - uses HasParent
    ├── Doctor (Modules\Application\Models\Doctor) - uses HasParent  
    └── Admin (Modules\Application\Models\Admin) - uses HasParent
```
...
### Modulo Application (User)
Aggiunge trait specifici per il dominio sanitario:

```php
// In Application\Models\User
...
La `UserFactory` è implementata **nel modulo Application** perché:

1. **Domain Specificity**: I dati sono specifici del dominio sanitario
2. **Enum Integration**: Usa `UserTypeEnum` e `UserState` del modulo Application
...
```php
// Factory nel modulo Application
namespace Modules\Application\Database\Factories;

class UserFactory extends Factory
{
    protected $model = \Modules\Application\Models\User::class;
    ...
    public function definition(): array
    {
        return [
            ...
            // Campi User Application (specifici dominio)
...
| BaseUser (User Module) | Application User | Usage |
|------------------------|----------------|-------|
...
// Application User - Domain-specific casts
protected function casts(): array
{
...
- **Application User**: Campi specifici del dominio sanitario
- **STI Children**: Campi altamente specializzati per tipo

### 2. Factory Responsibility

- **UserFactory in Application**: Genera dati completi per testing del dominio
...
- Modulo Application: Specifics per dominio sanitario
- Clear boundaries e responsibilities

### 3. Testing Flexibility
- Test generici nel modulo User
- Test specifici sanitari nel modulo Application
- Factory supporta entrambi i livelli

### 4. Maintenance
- Changes al BaseUser automaticamente ereditati
- Healthcare-specific changes isolati nel modulo Application
- Factory evolution indipendente

## Links to Documentation

### Application Module
- [UserFactory Improvements Analysis](../Application/docs/factories/UserFactory-improvements-analysis.md)
- [Model Architecture](../Application/docs/model-architecture.md)
- [STI Implementation](../Application/docs/model-inheritance.md)
