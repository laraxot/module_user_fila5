...
## Current Issue
The admin UI for users (`/application/admin/users`) shows entries but the list is hidden by CSS rules applied to the datatable wrapper. The rows appear as text nodes without actual row styling, making the list unreadable.
...
## Verification
- Open `/application/admin/users` and confirm each user row displays avatar, name, email, role, and actions.
