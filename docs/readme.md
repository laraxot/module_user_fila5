...
- [PHPStan Analysis](./phpstan-level10-user-fixes.md) - Report di conformità Level 10 per questo modulo
...
│   └── Actions/              # Business logic (Spatie Queueable Actions, no Services/ dir)
│       ├── User/CreateUserAction.php
│       ├── User/UpdateUserAction.php
│       └── Team/GetUserTeamsOptionAction.php
├── database/
...
> **Correzione di accuratezza**: questa sezione descriveva in precedenza classi
> `AuthService`, `PermissionService`, `TenantService` che **non esistono** in
> `Modules/User/app` — questo modulo non ha una directory `Services/`. La
> business logic vive in `app/Actions/**` come Spatie Queueable Actions (metodo
> `execute()`), per regola di progetto (vedi
> `bashscripts/ai/wiki/rules/no-services-rule.md`). Le operazioni di ruolo e
> permesso (`assignRole`, `givePermissionTo`, ecc.) restano chiamate dirette ai
> metodi Spatie sul model, senza wrapper dedicati.

**Esempi reali** (`Modules/User/app/Actions/`):
```php
// User/CreateUserAction.php, UpdateUserAction.php, DeleteUserAction.php
(new CreateUserAction())->execute($data);

// User/GetNewPasswordAction.php
(new GetNewPasswordAction())->execute($user);

// Team/GetUserTeamsOptionAction.php
(new GetUserTeamsOptionAction())->execute($user);

// Authentication/GetAuthenticationLogQueryForAuthenticatableAction.php
(new GetAuthenticationLogQueryForAuthenticatableAction())->execute($user);
```

#### Multi-tenancy

Gestita tramite i trait `HasTenants` / `BaseInteractsWithTenant` sul model, non
tramite una classe `TenantService`.
...
> **Nota di accuratezza**: le sottocartelle `architecture/auth/`, `auth/`,
> `authorization/`, `development/`, `security/`, `troubleshooting/` citate in
> precedenza in questa sezione non esistono in `Modules/User/docs`. I link sono
> stati sostituiti con i file realmente presenti che coprono gli stessi temi.
> Indice completo e aggiornato: **[00-INDEX.md](./00-INDEX.md)**.

### Architecture
- [BaseUser Architecture](./baseuser.md) - User model design
- [Permissions System](./permissions.md) - RBAC architecture
- [Passport Model Wrappers](./passport-model-wrappers.md) - OAuth model mapping

### Authentication
- [Authentication Flow](./authentication.md) - Login/logout process
- [Passport & SSO](./passport-integration.md) - OAuth2 / SSO integration
- [Socialite + Microsoft OAuth](./socialite-microsoft-integration.md) - Social login setup
- [Two-Factor Auth](./2fa-guide.md) - 2FA implementation

### Development
- [Model Inheritance](./model-inheritance-rules.md) - STI implementation rules
- [Testing Identity](./testing.md) - Auth/authorization test guide

### Security
- [No Log in Production](./no-log-in-production.md) - Logging rule for Actions/Models/Widgets
- [Over-engineering Audit](./ponytail-audit-over-engineering.md) - Unused policy stubs and contracts
