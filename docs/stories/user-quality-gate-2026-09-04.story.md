...
### Acceptance criteria
- [x] Rimuovere assegnazioni locali inutilizzate senza alterare gli effetti collaterali.
- [x] Rinominare variabili troppo brevi dove il significato resta invariato.
- [x] Rieseguire PHPStan, PHPMD e PHPInsights sul modulo User.
- [x] Chiudere il gate git locale; il push resta bloccato finché il remote `laraxot` del modulo non punta a
  `git@github.com:provtv/module_user_fila5.git` come dichiarato da `gitmodules.ini`.

### Modifiche
- Rimosse variabili inutilizzate in tre comandi, nel controller Upgrade e nelle azioni bulk Passport.
- Conservate le chiamate con effetti collaterali a `RevokeAllUserTokensAction`.
- Eliminati assegnamenti inutilizzati nei listener di autenticazione e rinominate variabili brevi.

### Esito gate
- PHPStan `Modules/User`: **0 errori**.
- PHPMD: exit `2` con rilievi residui preesistenti; i file di questo batch non compaiono più nell'output.
- PHPInsights: exit `0`; residui globali del modulo documentati nell'output `/tmp/user-phpinsights-after.txt`.
- PHPMD output completo: `/tmp/user-phpmd-after.txt` (448 finding lines residue).
- Il checkout è un'unica repository root (`git rev-parse --show-toplevel`), non una repository User
  separata. I remote `origin` e `laraxot` puntano entrambi erroneamente a `base_ptv_fila5`, mentre
  `gitmodules.ini` dichiara `module_user_fila5`: nessun push eseguito per evitare di pubblicare sul repo
  sbagliato.
