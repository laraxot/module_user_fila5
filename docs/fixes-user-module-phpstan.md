...
- `tests/Traits/HasUserTestCase.php`: dichiarava `@property User $user` solo in PHPDoc, senza `use` per la classe `User` né una property reale — PHPStan risolveva `User` nel namespace sbagliato. Aggiunta `use Modules\Xot\Contracts\UserContract;` + `protected User $user;` reale.
