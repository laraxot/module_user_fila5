...
uksort($sortedFiles, function (mixed $a, int|string $b) use ($allFiles) {
    $aStats = $allFiles[$a];
    $bStats = $allFiles[$b];
...
$uncoveredFiles = array_filter($allFiles, fn (mixed $stats) => 0 === $stats['elements']['covered'] && $stats['elements']['total'] > 0);
