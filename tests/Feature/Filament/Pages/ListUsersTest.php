...
describe('List Users', function (): void {
    ...
    test('list users page has correct table columns', function (): void {
        /** @var TestCase $this */
        $listUsersPage = $this->requireListUsersPage();
        $columns = $listUsersPage->getTableColumns(); // @phpstan-ignore method.deprecated (hook di progetto: la deprecazione e ereditata per nome dal prototipo Filament 5, il codice eseguito e il nostro — story 16.12)
    ...
    test('list users page has correct table filters', function (): void {
        /** @var TestCase $this */
        $listUsersPage = $this->requireListUsersPage();
        $filters = $listUsersPage->getTableFilters(); // @phpstan-ignore method.deprecated (hook di progetto: la deprecazione e ereditata per nome dal prototipo Filament 5, il codice eseguito e il nostro — story 16.12)
    ...
    test('list users page has correct table actions', function (): void {
        /** @var TestCase $this */
        $listUsersPage = $this->requireListUsersPage();
        $actions = $listUsersPage->getTableActions(); // @phpstan-ignore method.deprecated (hook di progetto: la deprecazione e ereditata per nome dal prototipo Filament 5, il codice eseguito e il nostro — story 16.12)
